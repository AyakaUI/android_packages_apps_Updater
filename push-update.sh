...
temporary_directory=$(mktemp -d "${TMPDIR:-/tmp}/ayaka-updater.XXXXXX") || \
...
python3 "$script_directory/tools/ayaka_feed.py" generate-ota "$zip_path" \
...
adb_cmd shell "test -d '$updates_dir'" || \
    die "$updates_dir does not exist; boot a build containing init.ayaka-updater.rc"
