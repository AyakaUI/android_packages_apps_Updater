...
SPDX-FileCopyrightText: 2026 AyakaUI
...
`net.ayaka.ota`. Add the `Updater` module to the product packages. Its Soong definition pulls in
the following required modules automatically:

- `default-permissions_net.ayaka.ota`, which grants the runtime notification permission by
  default;
- `init.ayaka-updater.rc`, which creates `/data/system_updates` as `system:cache` with mode
  `0770` and required file-based encryption.

The privileged permissions that require allowlisting are declared in `app/net.ayaka.ota.xml`.
...
It must also retain the `seapp_contexts` rule that assigns platform-signed `net.ayaka.ota` to
...
```text
https://raw.githubusercontent.com/AyakaUI-AOSP/official_devices/{branch}/API/updater/{device}.json
```
...
`tools/ayaka_feed.py` uses only the Python standard library. It reads authoritative values from
the OTA ZIP and computes size and SHA-256 itself:

```sh
tools/ayaka_feed.py generate-ota AyakaUI_device-17.0-build.zip \
  --url https://downloads.example.org/device/AyakaUI_device-17.0-build.zip \
  --version 17.0 \
  --output device.json

tools/ayaka_feed.py validate-ota device.json \
...
```text
https://raw.githubusercontent.com/AyakaUI-AOSP/official_devices/{branch}/API/updater/changelogs/{device}.md
```
...
`ro.custom.device`, validates OTA metadata through `tools/ayaka_feed.py`, computes SHA-256, checks
