...
MODULE_PATH = Path(__file__).parents[1] / "ayaka_feed.py"
SPEC = importlib.util.spec_from_file_location("ayaka_feed", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
ayaka_feed = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(ayaka_feed)


class AyakaFeedTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def create_ota(self, metadata_overrides: dict[str, str] | None = None) -> Path:
        ...
        with zipfile.ZipFile(artifact, "w") as archive:
            archive.writestr(ayaka_feed.OTA_METADATA_PATH, rendered)
            archive.writestr("payload.bin", b"payload")
        return artifact

    def test_generate_and_validate_ota_feed(self) -> None:
        artifact = self.create_ota()
        feed = ayaka_feed.build_ota_feed(
            artifact,
            "https://downloads.ayaka.net/device/update.zip",
            "17.0",
        )

        ayaka_feed.validate_ota_feed(feed, artifact=artifact)
        update = feed[0]
        self.assertEqual(1781858358, update["datetime"])
        self.assertEqual(ayaka_feed.sha256_file(artifact), update["files"][0]["sha256"])
    ...
    def test_ota_feed_rejects_more_than_one_file(self) -> None:
        ...
        with self.assertRaisesRegex(ayaka_feed.ValidationError, "exactly one"):
            ayaka_feed.validate_ota_feed(feed)
    ...
    def test_ota_feed_rejects_uppercase_hash(self) -> None:
        ...
        with self.assertRaisesRegex(ayaka_feed.ValidationError, "lowercase"):
            ayaka_feed.validate_ota_feed(feed)
    ...
    def test_ota_feed_rejects_unknown_keys(self) -> None:
        ...
        with self.assertRaisesRegex(ayaka_feed.ValidationError, "unknown keys"):
            ayaka_feed.validate_ota_feed(feed)
    ...
    def test_ota_ranges_must_fit_artifact(self) -> None:
        ...
        with self.assertRaisesRegex(ayaka_feed.ValidationError, "exceeds package size"):
            ayaka_feed.build_ota_feed(
                artifact,
                "https://downloads.ayaka.net/device/update.zip",
                "17.0",
            )
    ...
    def test_artifact_comparison_detects_hash_mismatch(self) -> None:
        ...
        with self.assertRaisesRegex(ayaka_feed.ValidationError, "does not match artifact"):
            ayaka_feed.validate_ota_feed(feed, artifact=artifact)
